...
<?php 
    ...
    $selectIdComprador = "SELECT cv.id_comprador FROM compra_venda cv WHERE cv.id = ". $_GET["idVenda"];
    $comprador = mysqli_fetch_assoc(mysqli_query($conn, $selectIdComprador));
    if($_SESSION["idComprador"] != $comprador["id_comprador"]){
        redirect("../page_gerPedidos.php");
    }

    // Utilizado para definir os botões do header
    $tipoPagina = "formUpdateCompra";
    ...
    if(!(is_null($dhPagamento)) && is_null($dhRecebimento)){
        $opcoes = "<button id=\"cPagamento\" class=\"w3-large w3-right w3-button w3-green w3-round\"><i class=\"fa-solid fa-check\"></i> Confirmar recebimento</button>";
    }else{
        $opcoes = "";
    }
?>
